...
class ZenAdapter():
    ...
    def devices(self, name):
        ...
        query = Or(idOrTitleQuery, Eq('getDeviceIp', name))
        brains = zcatalog.evalAdvancedQuery(query)
        brains += self.dmd.Networks.ipSearch.evalAdvancedQuery(idGlob)
        return [ b.getObject() for b in brains ]

    def components(self, device, component):
        log.debug('Searching for component %s in device %s' % (component, device.id))
        compList=[]
        for comp in device.getDeviceComponents():
            if component.replace('.','/') in comp.absolute_url_path():
